...
public class SuiteRoomFormController implements Initializable {
	...
	public void handleSubmitClick(ActionEvent event) {
		...
		try {
			if (inputRoomId.length() == 0 || inputFeatureSummary.length() == 0) {
				throw new InvalidInputException("Error: Fields can not be empty.");
			}

			CityLodge.getInstance().performAddSuiteRoom(inputRoomId, inputFeatureSummary);
			FxUtils.showInfoAlert("Suite Room Added Successfully", null);
			window.close();
